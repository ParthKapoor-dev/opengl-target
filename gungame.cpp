...
void fireShurikenStart(character* shurikenCharacter) {
    ...
    else if (shurikenCharacter->trajectoryYrotation>0)
        endX = shurikenCharacter->translation->z*tan(shurikenCharacter->trajectoryYrotation*toRad)-60;
    else
        endX = shurikenCharacter->translation->z*tan(shurikenCharacter->rotation->a*toRad)-70;
...
void characterHit() {
    ...
    // Use nanosleep instead of for loop for delay
    struct timespec ts;
    ts.tv_sec = 0;
...
void setupCamera() {
    ...
    glLoadIdentity();
    glOrtho(1, 1, -1, 1, -1, 1);
    gluPerspective(80, static_cast<double>(windowWidth) / windowHeight, 0.001, 1000);
